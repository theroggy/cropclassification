...
def write_full_report(parcel_predictions_csv: str
    ...
    classes = sorted(np.unique(np.append(df_predict[gs.class_column].unique()
                                         , df_predict[gs.prediction_column].unique())))
    ...
    with open(output_report_txt, 'w') as outputfile:
        ...
        message = f"General OA: {oa} of the items with a prediction"
        logger.info(message)
        outputfile.write(f"{message}\n")
        ...
        oa = sklearn.metrics.accuracy_score(df_predict_accuracy_best_case[gs.class_column]
                                            ...
                                            , normalize=True
                                            , sample_weight=None) * 100
        message = f"Best case OA: {oa} of the items with a prediction that are not in classes UNKNOWN nor IGNORE_*"
        logger.info(message)
        outputfile.write(f"{message}\n")
        ...
        if parcel_classes_test_csv is not None:
            ...
            df_parcel_all_test_no_predict = \
                    df_parcel_all_test[~df_parcel_all_test.index.isin(df_predict.index)]
            ...
            if nb_parcel_test_no_predict > 0:
                count_per_class = (df_parcel_all_test_no_predict
            ...
            if parcel_ground_truth_csv is not None:
                ...
                df_gt = df_all_with_pred.join(df_parcel_ground_truth, how='inner', rsuffix='_GT')
                logger.debug('Number of ground truth parcels that also have a prediction: {len(df_gt)}')
                ...
                values = df_gt.apply(get_prediction_quality, args=[gs.prediction_column], axis=1)
                prediction_quality_column_name = f"prediction_quality_{gs.prediction_column}"
                df_gt.insert(loc=2, column=prediction_quality_column_name, value=values)

                prediction_quality_cons_column = f"prediction_quality_{gs.prediction_cons_column}"
                values = df_gt.apply(get_prediction_quality, args=[gs.prediction_cons_column], axis=1)
                df_gt.insert(loc=2, column=prediction_quality_cons_column, value=values)
                df_gt.to_csv(output_report_txt + "groundtruth_pred_quality_details.csv")

                # First write the result for the standard predictions
                count_per_class = (df_gt.groupby(prediction_quality_column_name, as_index=False)
                                   .size().to_frame('count'))
                values = 100*count_per_class['count']/count_per_class['count'].sum()
                count_per_class.insert(loc=1, column='pct', value=values)
                with pd.option_context('display.max_rows', None, 'display.max_columns', None):
                    message = f"Prediction quality overview, for {len(df_gt)} predicted cases in ground truth:\n{count_per_class}"
                    logger.info(message)
                    outputfile.write(f"{message}\n")

                # Now write the result for the consolidated predictions
                count_per_class = (df_gt.groupby(prediction_quality_cons_column, as_index=False)
                                   .size().to_frame('count'))
                values = 100*count_per_class['count']/count_per_class['count'].sum()
                count_per_class.insert(loc=1, column='pct', value=values)
                with pd.option_context('display.max_rows', None, 'display.max_columns', None):
                    message = f"Prediction quality cons overview, for {len(df_gt)} predicted cases in ground truth:\n{count_per_class}"
                    logger.info(message)
                    outputfile.write(f"{message}\n")
        ...
        # Write extended confusion matrix and make sure all rows and collumns are written to the
        # output file...
