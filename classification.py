...
def train_and_test(input_parcel_classes_train_csv: str
    ...
    if (force == False and os.path.exists(output_classifier_filepath) and os.path.exists(output_test_predictions_csv)):
...
#    logger.setLevel(logging.DEBUG)
    ...
    df_train = df_train[[gs.id_column, gs.class_column]].join(df_classification_data, how = 'inner', on = gs.id_column)

    # First train the classifier
    _train(df_train = df_train
    ...
    df_test = df_test[[gs.id_column, gs.class_column]].join(df_classification_data, how = 'inner', on = gs.id_column)
...
def predict(input_parcel_classes_csv: str
    ...
    # If force == False Check and the output file exists already, stop.
    if force == False and os.path.exists(output_predictions_csv):
        logger.warning(f"predict: output file already exists and force == False, so stop: {output_predictions_csv}")
        return
    ...
    # Join the columns of df_classification_data that aren't yet in df_train
    logger.info("Join train sample with the classification data")
    df_in = df_in[[gs.id_column, gs.class_column]].join(df_classification_data, how = 'inner', on = gs.id_column)
...
def _train(df_train: pd.DataFrame
    ...
    """
    Train a classifier, apply it to the test dataset and write the results of the classification to the output file.
    """

    # If force == False Check and the output file exists already, stop.
    if force == False and os.path.exists(output_classifier_filepath):
...
def _predict(df_input: pd.DataFrame
    ...
    # If force == False Check and the output file exists already, stop.
    if force == False and os.path.exists(output_parcel_predictions_csv):
        logger.warning(f"predict: output file already exists and force == False, so stop: {output_parcel_predictions_csv}")
        return
    ...
    id_class_top3 = np.concatenate([df_input[[gs.id_column, gs.class_column]].values, top3_pred], axis=1)

    # Convert to dataframe, combine with input data and write to file
    df_top3 = pd.DataFrame(id_class_top3, columns = [gs.id_column, gs.class_column, 'pred1', 'pred2', 'pred3', 'pred1_prob', 'pred2_prob', 'pred3_prob'])
    ...
    df_top3.to_csv(output_parcel_predictions_csv, float_format = '%.10f', encoding = 'utf-8', index = False)
    ...
    df_proba.to_csv(f"{filepath_noext}_detailed{ext}", float_format = '%.10f', encoding = 'utf-8', index = False)
