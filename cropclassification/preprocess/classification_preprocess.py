...
def prepare_input(
    ...
    if force is False and os.path.exists(output_parcel_path) is True:
        logger.warning(
            f"prepare_input: output file already exists and force == False, so stop: {output_parcel_path}"
        )
        return
...
def create_train_test_sample(
    ...
    if (
        ...
        logger.warning(
            f"create_train_test_sample: output files already exist and force == False, so stop: {output_parcel_train_path}, {output_parcel_test_path}"
        )
        return
