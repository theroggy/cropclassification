...
def get_best_model(model_dir: str,
    ...
    if(acc_metric_mode not in acc_metric_mode_values):
        raise Exception(f"Invalid value for mode: {acc_metric_mode}, should be one of {acc_metric_mode_values}")
...
def save_and_clean_models(model_save_dir: str,
    ...
    if verbose is True or debug is True:
        best_model = get_best_model(model_save_dir, acc_metric_mode, model_save_base_filename)
